...
document.addEventListener("click", function(event){
	if(event.target.classList.contains('filter')){
		if($select.value === ""){
			...
			tableRendering(filter($select.value));
		}
	} 
});

function filter ( marker ) {
	return data.filter( elem => "" + elem.type === marker);
}

function tableRendering ( dataArray ) {
	let table = `<tr>
						<th>price</th>
						<th>type</th>
						<th>area</th>
...
function getAllTypes ( dataArray ) {
	let allTypesArray = [];
	for(let obj of dataArray){
		let coincidence = false;
		for(let type of allTypesArray){
			if(obj.type === type) coincidence = true;
		}
		if(!coincidence) allTypesArray.push(obj.type);
	}
	return allTypesArray.sort();
}

function fillSelect ( arrayOfSelectElements ) {
	let select = `<option value=""></option>`;
	for(let elem of arrayOfSelectElements){
		select += `<option value="${elem}">${elem}</option>`;
	}
	$select.innerHTML = select;
}
fillSelect(getAllTypes(data));
